...
def model(files, batch_size, epochs, steps_per_epoch, gpus, num_still_images, model_dir):
	...
	training_generator = DataGenerator(train_files, num_still_images, batch_size)
	validation_generator = DataGenerator(val_files, num_still_images, batch_size)


	# Audio encoder
	input_audio = Input(shape=(12, 35, 1)) 

	x = convolution(input_audio, 64, 3) # 12, 35, 64
	x = convolution(x, 128, 3) 			# 12, 35, 128
	x = MaxPooling2D((3, 3), strides = (1, 2), padding='same')(x) # 12, 18, 128
	x = convolution(x, 256, 3) # 12, 18, 256
	x = convolution(x, 256, 3) # 12, 18, 256
	x = convolution(x, 512, 3) # 12, 18, 512
	x = MaxPooling2D((3, 3), strides = 2, padding='same')(x) # 6, 9, 512
	x = Flatten()(x) # 27648 
	x = Dense(512, activation='relu')(x) # 512
	encoded_audio = Dense(256, activation='relu')(x) # 256
	...
	x = convolution(input_identity, 96, 7, 2) # 56, 56, 96
	x_skip1 = MaxPooling2D((3, 3), strides=2, padding='same')(x) # 28, 28, 96
	x_skip2 = convolution(x_skip1, 256, 5, 2) # 14, 14, 256
	x_skip3 = MaxPooling2D((3, 3), strides=2, padding='same')(x_skip2) # 7, 7, 256
	x = convolution(x_skip3, 512, 3) # 7, 7, 512
	x = convolution(x, 512, 3) # 7, 7, 512
	x = convolution(x, 512, 3) # 7, 7, 512
	x = Flatten()(x) # 25088
	x = Dense(512, activation='relu')(x) # 512
	encoded_identity = Dense(256, activation='relu')(x) # 256
